...
async function updateDashboardWithAll47() {
  const dbProducts = await db.product.findMany()
  console.log('Fetched products count:', dbProducts.length)
  ...
  const formattedProducts = dbProducts.map((p) => {
    ...
    return {
      ...
      status: 'جاهز للنشر',
      thumb: imgs[0],
      short_description: p.shortDescription || (p.description ? p.description.substring(0, 100) : 'منتج تجميل وعناية ممتاز عالي الجودة متاح في قطاع غزة.'),
      long_description: p.description || p.shortDescription || 'منتج تجميل عالي الجودة متوفر بخصم مميز وخدمة توصيل سريعة لجميع مناطق قطاع غزة.',
